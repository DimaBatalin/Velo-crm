...
`POST /parts` не всегда создаёт новую строку: одинаковые позиции объединяются (см. §7).
...
- **Объединение позиций склада**: `POST /parts` сначала ищет уже существующую
  позицию с тем же названием (без учёта регистра и краевых пробелов), той же
  закупочной ценой, той же ценой продажи и тем же владельцем. Если находит —
  прибавляет `quantity` к ней и возвращает её же (статус остаётся `201`),
  новая строка не создаётся. Категория, артикул, поставщик, заметки и
  `min_stock` на совпадение не влияют; пустые `category`/`sku`/`supplier`
  дозаполняются данными новой поставки, заполненные не перетираются.
  Правило работает и для формы на странице «Запчасти», и для «Создать и
  списать» внутри ремонта. `PUT /parts/{id}` объединение **не** выполняет.
